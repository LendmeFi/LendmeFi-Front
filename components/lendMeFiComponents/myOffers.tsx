...
import { ListingDetails, nftData, OfferDetails } from "@/types/lendingDetails";
import { BrowserProvider } from 'ethers'
import { useWeb3ModalProvider } from '@web3modal/ethers/react'
import { getNftListingsByAddress, getNftOffersByAddress } from "@/app/firebaseService";
...
const myOffers: React.FC = () => {
    ...
    const [offersData, setOffersData] = useState<OfferDetails[]>([]);
    ...
    async function offersDataByAddress() {
        const signer = await wProvider.getSigner();
        const userAddress = await signer.getAddress();
        const offerData: OfferDetails[] = await getNftOffersByAddress(userAddress);
        return offerData;
    }
    ...
    useEffect(() => {
        const fetchDataAndPictures = async () => {
            const data = await offersDataByAddress();
            setOffersData(data);
            if (data.length > 0) {
                setHasNfts(true);
            }
    ...
    function handleOnPress(nft: OfferDetails): void {
